        ...
        $(function () {
            ...
            $("button[name='revise']").click(function () {
                ...
                $.ajax({
                    type:"get",
                    dataType : "json",
                    url : "${pageContext.request.contextPath}/newsTypeAction",
                    data : {
                        updateId : $(this).val(),
                        flag : "revise"
                    },
...
">
        ...
        <div class="container">
            ...
            <div class="updateUser" id="updateUser">
                ...
                <div class="updateUserBox">
                    ...
                    <form class="form-horizontal" action="${pageContext.request.contextPath}/newsTypeAction?flag=superSearchUpdateOther&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}"
                          method="post">
                        </c:if>
                        <c:if test="${no eq issearch}">
                        <form class="form-horizontal" action="${pageContext.request.contextPath}/newsTypeAction?flag=superUpdateOther&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}"
                              method="post">
                            </c:if>
    ...
    padding-left: 20px;">
                            ...
                            <a href="${pageContext.request.contextPath}/newsTypeAction?flag=addNewsType"><div style="
    width:  80px;
    border: 1px solid white;
    text-align:  center;
    color:  white;
    ...
    font-weight:  bold;
">添加栏目</div></a>
                            <form action="${pageContext.request.contextPath}/newsTypeAction?flag=superCompoundSearch&showRowCount=${showRowCount}"
                                  method="post"
                                  style="padding-left: 500px;">
                                <input type="hidden" name="id">
                                <input type="text" value="${searchTypeName}" placeholder="请输入栏目..." style="color: white"
                                       name="searchTypeName">

                                <input value="${searchSort}" name="searchSort" type="text"
                                       placeholder="请输入编号..."
                        ...
                        <table id="fresh-table" class="table">
                            <c:if test="${ok eq issearch}">
                            <form action="${pageContext.request.contextPath}/newsTypeAction?flag=batch&superCompoundSearch=superCompoundSearch&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}"
                                  method="post">

                                </c:if>
                                <c:if test="${no eq issearch}">
                                <form action="${pageContext.request.contextPath}/newsTypeAction?flag=batch&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}"
                                      method="post">

                                    </c:if>
                                    <thead>
                                    <th data-field="id">ID</th>
                                    <th data-field="id">栏目ID</th>
                                    <th data-field="salary" data-sortable="true">栏目名称</th>
                                    <th data-field="salary" data-sortable="true">管理</th>
                                    <th data-field="country" data-sortable="true" style="text-align: left"><input type="submit" value="批量删除">
                                    </th>

                                    </thead>
                                    <tbody>
                                    <!--//for循环-->
                                    <c:forEach items="${findAllNewsType }" var="list">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <c:if test="${ok eq issearch}">
                                                ...
                                                <button type="button" onclick="window.location.href='newsTypeAction?flag=delete&superCompoundSearch=superCompoundSearch&id=${list.id }&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}'"
                                            ...
                                            <c:if test="${no eq issearch}">
                                                <button type="button" name="revise" value="${list.id}" style="background-color: cadetblue;">修改</button>
                                                <button type="button" onclick="window.location.href='newsTypeAction?flag=delete&id=${list.id }&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}'"
                                                        style="    background-color: slateblue;">删除
                                                </button>
                        ...
                        <div class="toolbar">
                            ...
                            <script type="text/javascript">
                                window.onload = function () {
                                    ...
                                    showRowCount.onchange = function () {
                                        if (${ok eq issearch}) {
                                            location.href = "${pageContext.request.contextPath}/newsTypeAction?flag=superCompoundSearch&showRowCount=" + this.value + "&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}";
                                        } else {

                                            location.href = "${pageContext.request.contextPath}/newsTypeAction?flag=superSearchTypeid&showRowCount=" + this.value;
                                        }
                                    }
                            ...
                            <c:if test="${pageNow != 1}">
                                <c:if test="${ok eq issearch}">
                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superCompoundSearch&pageNow=1&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">首页</a>

                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superCompoundSearch&pageNow=${requestScope.pageNow-1 }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">上一页</a>
                                </c:if>
                                <c:if test="${no eq issearch}">
                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&pageNow=1&showRowCount=${showRowCount}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">首页</a>

                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&pageNow=${requestScope.pageNow-1 }&showRowCount=${showRowCount}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">上一页</a>
                                </c:if>
                            ...
                            <c:if test="${pageNow>3}">
                                ...
                                <c:if test="${ok eq issearch}">
                                    ...
                                    text-align: center;
                                    border-radius: 35px;'
                                               href="${pageContext.request.contextPath }/newsTypeAction?flag=superCompoundSearch&pageNow=${i }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}">${i }</a>
                                        </c:if>
                                        <c:if test="${i!=requestScope.pageNow }">
                                ...
                                <c:if test="${no eq issearch}">
                                    ...
                                    text-align: center;
                                    border-radius: 35px;'
                                               href="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&pageNow=${i }&showRowCount=${showRowCount}">${i }</a>
                                        </c:if>
                                        <c:if test="${i!=requestScope.pageNow }">
                                            <a style="display:inline-block;color: white;margin-left: 20px;width: 35px;
                                    height: 35px;
                            ...
                            <c:if test="${(requestScope.pageNow != pageCount) && pageCount != 0}">
                                ...
                                <c:if test="${no eq issearch}">
                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&pageNow=${requestScope.pageNow+1 }&showRowCount=${showRowCount}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">下一页</a>
                                    <a href="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&pageNow=${requestScope.pageCount }&showRowCount=${showRowCount}"
                                       style="color: white;margin-left: 20px;width: 20px;height: 20px;font-size: 18px;">尾页</a>
                                </c:if>
                            </c:if>
                            <c:if test="${(requestScope.pageNow == pageCount) || pageCount == 0}">
                            ...
                            <c:if test="${ok eq issearch}">
                                <form action="${pageContext.request.contextPath }/newsTypeAction?flag=superCompoundSearch&pageNow=${requestScope.pageNow }&showRowCount=${showRowCount}&searchUserName=${searchUserName}&searchDepID=${searchDepID}&searchUserCode=${searchUserCode}&searchUserSex=${searchUserSex}&searchUserAge=${searchUserAge}&searchUserPower=${searchUserPower}"
                            ...
                            <c:if test="${no eq issearch}">
                                <form action="${pageContext.request.contextPath }/newsTypeAction?flag=superSearchTypeid&showRowCount=${showRowCount}"
                                      method="post" style="display: inline-block">
                                    <input type="text" name="page" style="width: 50px;margin-left: 20px;color: white;"
                                           placeholder="       页"><input
                                        type="submit" value="跳转" style="width: 50px;color: white">
